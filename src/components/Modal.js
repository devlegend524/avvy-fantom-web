  ...
  render() {
    ...
    return (
      <div className={`duration-150 transition-all flex items-center justify-center z-10 fixed top-0 left-0 w-full h-full ${open ? 'pointer-events-all opacity-100' : 'pointer-events-none opacity-0'}`}>
        ...
        <div onClick={(e) => e.stopPropagation()} className='m-4 bg-white dark:bg-gray-900 rounded max-w-screen-md max-h-screen w-full border-gray-200 dark:border-gray-700 border p-4 shadow relative z-20 overflow-y-scroll'>
          {this.props.title ? (
